...
const API = {
  ...
  updateRiderStatus: (id, s)     => _patch(`/api/riders/${id}/status`, { status: s }),
  updateRiderLocation:(id, lat, lon) => _patch(`/api/riders/${id}/location`, { current_lat: lat, current_lon: lon }),
  deleteRider:       (id)        => _del(`/api/riders/${id}`),
  nearbyRiders:      (whId, km)  => _get(`/api/riders/nearby/${whId}?radius=${km || 3}`),
...
async function _get(p)       { return (await fetch(BACKEND_URL + p)).json(); }
async function _post(p, b)   { return (await fetch(BACKEND_URL + p, { method:"POST",   headers:{"Content-Type":"application/json"}, body:JSON.stringify(b) })).json(); }
async function _put(p, b)    { return (await fetch(BACKEND_URL + p, { method:"PUT",    headers:{"Content-Type":"application/json"}, body:JSON.stringify(b) })).json(); }
async function _patch(p, b)  { return (await fetch(BACKEND_URL + p, { method:"PATCH",  headers:{"Content-Type":"application/json"}, body:JSON.stringify(b) })).json(); }
async function _del(p)       { return (await fetch(BACKEND_URL + p, { method:"DELETE" })).json(); }
function _qs(params) { if (!params) return ""; const s = new URLSearchParams(params).toString(); return s ? "?" + s : ""; }
